...
class RealDataEvaluator:
    ...
    def load_real_casme2_sequences(self, labels_df: pd.DataFrame) -> Tuple[torch.Tensor, torch.Tensor, np.ndarray, np.ndarray]:
        ...
        for idx, row in filtered_df.iterrows():
            ...
            try:
                ...
                if len(frames_seq) == (offset_frame - onset_frame + 1):
                    ...
                    print(f"⚠️  Incomplete frame sequence for {episode_id}")
    ...
    def run_real_evaluation(self) -> Dict:
        ...
        if real_data is None:
            ...
            print("🔧 Falling back to honest synthetic demonstration")
            
            # Fall back to honest evaluation
            honest_evaluator = HonestFaceSleuthEvaluation(self.data_dir, self.results_dir)
            return honest_evaluator.run_honest_evaluation()
...
if __name__ == "__main__":
    main()
                'base_uar': base_uar,
                'boosted_uar': boosted_uar,
                'accuracy_improvement': mean_boosted_accuracy - mean_base_accuracy,
                'uar_improvement': boosted_uar - base_uar
            },
            'total_samples': len(all_true_labels),
            'feature_dimension': fold_results[0]['feature_dimension'] if fold_results else 0
        }
        
        print(f"\n🎯 REAL CASME-II EVALUATION RESULTS:")
        print(f"   Base Accuracy: {mean_base_accuracy:.3f} ({mean_base_accuracy*100:.1f}%)")
        print(f"   Boosted Accuracy: {mean_boosted_accuracy:.3f} ({mean_boosted_accuracy*100:.1f}%)")
        print(f"   Accuracy Improvement: +{results['overall_metrics']['accuracy_improvement']*100:.1f}%")
        print(f"   Base UAR: {base_uar:.3f} ({base_uar*100:.1f}%)")
        print(f"   Boosted UAR: {boosted_uar:.3f} ({boosted_uar*100:.1f}%)")
        print(f"   UAR Improvement: +{results['overall_metrics']['uar_improvement']*100:.1f}%")
        print(f"   Total Samples: {results['total_samples']}")
        print(f"   Feature Dimension: {results['feature_dimension']}D")
        print(f"   Data Type: REAL CASME-II")
        
        # Save results
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        results_file = self.results_dir / f"real_casmeii_evaluation_{timestamp}.json"
